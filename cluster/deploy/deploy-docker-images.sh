...
function load_images_all() {
  ls $IMAGE_PATH/etcd* | while read image ; do cat $image | sudo docker load; done
  
  #cat $IMAGE_PATH/etcd-amd64-2.2.1.tar | sudo docker load
  
}

function load_images_separately() {
